...
## Из тикета 03 — разгрузка index.ts

- `installSmokeHooks(mainWindow: BrowserWindow, store: Store): void`
  (`src/main/smoke/index.ts`) — единственная точка входа smoke-обвязки,
  вызывается из `src/main/index.ts` один раз после `createWindow()`. Ничего
  не делает без `RH_SMOKE=1`.
- Каждый перенесённый сценарий — функция `run<Name>Flow(mainWindow,
  watchdog, ...)`, возвращающая `Promise<boolean>`: `true` — сценарий
  сработал (соответствующий `RH_SMOKE_*` был выставлен) и уже вызвал
  `app.exit(...)`; `false` — флаг не выставлен, идём к следующему сценарию.
  Не переизобретай этот паттерн — если понадобится новый smoke-сценарий,
  добавляй `runXFlow` в подходящий по теме файл под `src/main/smoke/` и
  один вызов в каскаде `smoke/index.ts`.
