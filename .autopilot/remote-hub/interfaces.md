...
## Из тикета 10 — настройки в левой панели, финал

- Настройки — встроенная панель в левом сайдбаре: кнопка «⚙ Настройки» в футере открывает sheet `.sidebar-settings-sheet` над футером (дерево остаётся видимым), закрытие — «✕» в шапке панели или повторный клик по ⚙. Состояние вида — `sidebarView: 'tree' | 'settings'` + `setSidebarView` в рендерер-сторе (`store.ts`); меню приложения (команда `settings`) тоже переключает панель.
- Форма вынесена в `SettingsForm.tsx` (Тема, Шрифт терминала + «Применить шрифт», Размер шрифта, Акцентный цвет, подтверждение удаления, восстановление вкладок). Модальный `SettingsDialog` удалён; тип `settings` убран из `DialogState`/`DialogRoot`.
- UI-фиксы из пользования: оверлей модалок `rgba(0,0,0,0.55) → 0.32` (смена темы видна за диалогом), кластер «⧉ Туннели / Σ Сниппеты + быстрое подключение» прижат к правому краю панели вкладок (убран второй auto-зазор), кнопка ⚙ убрана из панели вкладок, активное состояние кнопки «⚙ Настройки» — `.sidebar-footer .btn--active`.
- Smoke: `RH_SMOKE_THEME` — открытие панели из сайдбара → светлая тема → акцент → закрытие по ✕ (проверка возврата дерева); `RH_SHOT_SETTINGS` — скриншот панели. Скриншот темы — `RH_SHOT_PATH` при `RH_SMOKE_THEME`.
