...
export const HELP_SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    body: (
      <>
        <Lead>Три «тяжёлых» протокола ведут себя по-разному. Вот их поведение.</Lead>
        <SessionsMock />
        <H2>RDP</H2>
        <P>
          RDP встраивается прямо во вкладку: рабочий стол Windows открывается внутри приложения, следует за размером
          окна и закрывается вместе с вкладкой. Исключение — профили с полным экраном или несколькими мониторами:
          такой режим физически нельзя встроить, поэтому окно Remote Desktop открывается отдельно, и вкладка об этом
          честно сообщает.
        </P>
        <P>
          Разрешение и режим меняются прямо во вкладке: выпадающий список вверху выбирает разрешение, кнопка «⛶ Полный экран» разворачивает сессию на весь экран (окно откроется отдельно), а «Встроить во вкладку» возвращает её обратно.
        </P>
        <P>
      ...
      <>
        ...
        <Faq
          items={[
            ...
            {
              q: 'RDP открывается отдельным окном — почему?',
              a: (
                <>
                  Обычные сессии встроены во вкладку. Отдельным окном открываются только профили с полным экраном или
                  несколькими мониторами — такой режим физически нельзя встроить. Если окно вообще не появляется,
                  проверьте порт 3389, домен и опцию «Всегда спрашивать учётные данные» в профиле хоста.
                </>
              )
            },
            {
