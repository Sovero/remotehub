...
function SessionsMock(): React.JSX.Element {
  return (
    <Mock label="RDP, VNC и SFTP" w={640} h={300}>
      ...
      <T x={36} y={98} className="m-muted" size={11}>Запускается в отдельном</T>
      <T x={36} y={112} className="m-muted" size={11}>окне mstsc.exe.</T>
      <T x={36} y={136} className="m-muted" size={11}>Вкладка остаётся открытой,</T>
      <T x={36} y={150} className="m-muted" size={11}>пока не закрыть окно.</T>
      ...
      <>
        ...
        <Table
          ...
          rows={[
            ...
            ['RDP', '3389', 'Удалённый рабочий стол Windows (открывается в окне mstsc).'],
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <H2>RDP</H2>
        <P>
          RDP запускает системное окно Windows Remote Desktop (mstsc.exe) в отдельном окне. Вкладка при этом остаётся
          открытой: из неё можно запустить сессию заново или закрыть вкладку. Закрытие приложения не прерывает окно RDP.
        </P>
        <H2>VNC</H2>
        <P>
      ...
      <>
        ...
        <Faq
          items={[
            ...
            {
              q: 'RDP открывается в отдельном окне — это нормально?',
              a: (
                <>
                  Да, RDP использует системное окно mstsc.exe. Если оно не появляется, проверьте порт 3389, домен и
                  опцию «Всегда спрашивать учётные данные» в профиле хоста.
                </>
              )
            },
            {
